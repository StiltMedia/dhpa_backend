...
* All Environments
  ...
  * AWS_REFILE_BUCKET
    * S3 bucket name for image uploads (i.e. dhpa-photos-staging for staging)
...
* Use AWS S3 for image uploads
  ...
  * In the us-east-1 region (Virginia), create test, dev, staging, and production buckets according to the AWS_REFILE_BUCKET environment variable. Beware using production buckets outside of the production webserver.
* Use cloud66 for staging/production deployment (in the us-east-1 region; use t2.medium for production server size)
...
* Manually ensure that large file uploads are enabled:
  ...
  1. Edit the Nginx configuration to: <tt>client_max_body_size    300m;</tt>
...
Run <tt>rails s</tt> for development. Cloud66 scripts are already present.

== Maintenance & Backups

* The webserver and database server are both managed via cloud66 and hosted on AWS EC2. I have enabled daily automatic backups via cloud66, but you will need to manually verify that the backups are working and useable on a regular basis (every 3 months, for example.)
* Your photos are uploaded to Amazon S3; the "production" bucket is the important one. I've enabled versioning on it to avoid a disaster if things are accidentally deleted, but the entire bucket should be manually backed up every 3 months as well.

== Contributors

- Gustavo Pares https://twitter.com/staycreatived (http://staycreativedesign.com/)
- Will Bradley (will@zyphon.com)