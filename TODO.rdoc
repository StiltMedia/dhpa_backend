...
1. When a photo has names which are missing, visitors should be able to click a "Whats my Name" link under a photo to email the client the missing name - like http://www.patrickmcmullan.com/whoami/index.aspx
   * Question: does it need to be email? What if it just puts a message into a list on the admin panel?
1. Admin should be able to change the Price for each individual Folder
   * Question: what does "individual folder" mean? Changing prices for all photos in each event?
1. Assistant Photographers, Editors, Staff, etc. should only be able to:
   * Access the Events Tab (not Orders, Users, Settings)
   * Add or Delete an Event.
   * See the last 3-5 Events.
   * Question: can they edit those last 3-5 events? What about editing the event they just added 5 minutes ago? Is there any difference between Assistant Photographers, Editors, and Staff?
1. Admin can overwrite everything that the Assistant Photographers, Editors, Staff, do on the Website. (Unlock Passwords, adjust prices back to Default, etc.)
   * Question: By overwrite, do you mean simply make those changes, or prevent others from making changes once you've made them? For example if you and a photographer can both edit pricing on an item, you'll both be able to continue editing pricing on that item unless I program some sort of per-item "locked by Admin" functionality.
1. (0.5hr) The Logo on the Upper Left Corner has to be replaced (with the one that is attached in this email), or add this text manually "David Heischrek Photo Agency" below DHPA.com (in the same exact font as in the logo that is attached). The font is Dosis, medium, sharp.
1. (0.75hr) Add the Name to the Events that are rotating on the Top Feature Slide (same as BFA.com). Then take out the Welcome to DHPA.com.
1. (4hr) Re-add physical print options to checkout: selecting Sizes, Paper, and custom/default pricing of Prints, etc. Size options: 5x7, 8x10, 11x14, 16x20. Paper options: Luster & Deep Matte.
...
1. (2hr) After 1-5 minutes of visiting the website, a Pop-Up Box should appear asking visitors for their Email address
1. (0.5hr) Fix the Favicon. The way it looks now nobody can read what is says. Take out the .com in DHPA, and just put DHPA. And if DHPA is still not readable, then just make the Favicon a Photo Camera as a Logo (white camera, black background), or something that fits the look of my Website.
1. (0.75hr) When searching for photos (for example a person's Name), and the photos that are then found pop up, the Horizontal photos display the correct way, but the Vertical photos don't, and the heads are usually cut off.

* CDN for faster image serving
* Waiting on client
  1. Images to add to About page
  1. (1hr) Add Text about myself on the About page. In the middle, between the 4 photos (Studio, Video, Weddings, Social Media), and Have an Upcoming Event?
* Out of scope
